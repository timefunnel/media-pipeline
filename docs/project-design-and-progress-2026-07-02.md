...
这些值来自远端 `/opt/media-pipeline/.env` 或 compose 默认插值；`docker-compose.yml` 只保留变量引用。`MEDIA_PIPELINE_*_MSG_LIBRARY_ID` 和 `MEDIA_PIPELINE_*_MSG_ROOT_ID` 必须显式配置，Bot 只通过 MSG 管理员 API 工作，不再读取 MSG PostgreSQL。文档只记录变量存在，不记录任何 token/key/password。
...
说明：表中的 library/root id 均为占位符。当前代码要求按 `docs/deployment-guide.md` 显式填写真实 `library_id/root_id`；缺失或仍为 `REPLACE_WITH_` 时会直接报错，不再查询 MSG PostgreSQL 或静默选择 root。
...
- Bot 提交后会进入后台 115 状态轮询：
  ...
  - 任务超过 3600 秒仍未完成时自动取消 115 离线任务，并向 Bot 推送取消通知。
