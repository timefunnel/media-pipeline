...
class DanmakuMatchingPolicyTest(unittest.TestCase):
    ...
    def test_playback_match_uses_file_identity_and_prewarms_comments(self):
        ...
        self.assertEqual(matcher.tmdb_calls, [])
    ...
    def test_playback_match_falls_back_to_unique_tmdb_episode(self):
        from pipeline.bot import BotConfig, PipelineBotService
        ...
        class Matcher:
            ...
            def comments(self, *args, **kwargs):
                self.comment_calls.append((args, kwargs))
                return {"cached": False, "count": 100}

        matcher = Matcher()
        service = PipelineBotService(BotConfig("token", {700656624}))
        media = {
            "id": "media-2",
            "path": "/media/Kaijuu.S01E02.mp4",
            "tmdb_id": 207468,
            "season_num": 1,
            "episode_num": 2,
            "size_bytes": 1000,
        ...
        identity = {"file_hash": "b" * 32, "file_size": 1000, "prefix_bytes": 1000}

        with patch.object(service, "_require_danmaku_matcher", return_value=matcher), patch.object(
            service, "_load_media_detail", return_value=media
        ), patch("pipeline.bot.fetch_danmaku_file_identity", return_value=identity):
            result = service.danmaku_playback_match(
                "media-2", "https://cdn.example.test/episode-2.mp4", file_name="Kaijuu.S01E02.mp4"
            )

        self.assertEqual(matcher.tmdb_calls, [{"tmdb_id": "207468", "episode": 2}])
        self.assertEqual(result["match"]["episode_id"], "175500002")
        self.assertEqual([item["mode"] for item in result["match"]["attempts"]], ["hash", "tmdb"])
        self.assertFalse(result["match"]["cached"])
        self.assertEqual(matcher.comment_calls[0][0], ("175500002",))
        self.assertEqual(matcher.comment_calls[0][1]["match_mode"], "tmdb")
...
class TelegramBotTest(unittest.TestCase):
    ...
    def test_callback_submit_syncs_completed_115_share_immediately(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            candidate_id = store.save_candidate(
                ...
                candidate={
                    "title": "ABF-001",
                    "download_uri": "https://115.com/s/swabc123?password=xy99",
                    "source_kind": "115_share",
                    "rank": 1,
                },
            )
            ...
            service = FakeBotService(
                submit_response={
                    ...
                    "tasks": [submit_task],
                    "task_status": submit_task,
                },
                sync_progress=[{"msg_sync_status": "running", "msg_scan_status": "running"}],
                sync_response={
                    "msg_sync_status": "success",
            ...
            saved = store.load_task("share-swabc123")["task"]

        self.assertEqual(service.submit_calls, [("adult", "https://115.com/s/swabc123?password=xy99")])
        self.assertEqual(service.sync_calls, [("adult", "ABF-001", "share-swabc123")])
        self.assertEqual(saved["msg_sync_status"], "success")
        self.assertEqual(saved["msg_scan_status"], "success")
        self.assertEqual(saved["msg_scrape_status"], "success")
        self.assertEqual(saved["msg_media_id"], "media-1")
        self.assertEqual(saved["telegram_status_message_id"], 1001)
        self.assertEqual({edit["message_id"] for edit in telegram.edits}, {1001})
        self.assertIn("media-1", telegram.edits[-1]["text"])
    ...
    def test_subtitle_report_bulk_buttons_run_backfill_and_retry(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(operation_row[0]["text"], "批量补齐待补可尝试 3")
            self.assertEqual(operation_row[0]["callback_data"], "subbulk:pending:3")
            ...
            with patch("pipeline.bot.threading.Thread", ImmediateThread):
                ...
                bot.handle_update(
                    {
                        "callback_query": {
                            "id": "cb-subtitle-report-not-found",
                            "from": {"id": 700656624},
                            "message": {"chat": {"id": 9001}, "message_id": 1001},
                            "data": "subtitle_report:not_found:0",
                        }
                    }
                )
                retry_row = telegram.edits[-1]["reply_markup"]["inline_keyboard"][0]
                self.assertEqual(retry_row[0]["text"], "批量重试未找到 3")
                self.assertEqual(retry_row[0]["callback_data"], "subbulkr:not_found:3")
                bot.handle_update(
                    {
                        "callback_query": {
        ...
        self.assertEqual(service.subtitle_backfill_calls, [(3, False, "pending"), (3, True, "not_found")])
        self.assertEqual(telegram.answers[-3], {"callback_query_id": "cb-subtitle-backfill", "text": "开始补齐待补可尝试字幕"})
        self.assertEqual(telegram.answers[-2], {"callback_query_id": "cb-subtitle-report-not-found", "text": "正在刷新字幕统计"})
        self.assertEqual(telegram.answers[-1], {"callback_query_id": "cb-subtitle-backfill-retry", "text": "开始重试未找到字幕"})
        self.assertIn("成人库字幕补齐报表", telegram.edits[-1]["text"])
        self.assertIn("成人库字幕补齐：已完成", telegram.edits[-1]["text"])
        self.assertIn("范围：未找到", telegram.edits[-1]["text"])
    ...
    def test_subtitle_report_command_shows_counts_and_switches_bucket(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(keyboard[0][0]["callback_data"], "subbulk:pending:20")
    ...
    def test_subtitle_report_single_retry_button_retries_previous_attempt(self):
        ...
        self.assertEqual(retry_button["text"], "#1 重试")
...
class PipelineBotServiceTest(unittest.TestCase):
    ...
    def test_task_status_refreshes_for_malformed_access_token(self):
        from pipeline.bot import BotConfig, PipelineBotService
        ...
        class MalformedToken115Client:
            ...
            def get_offline_tasks(self, page=1):
                ...
                if self.token == "expired-token":
                    return {"state": False, "code": 40140123, "message": "access_token 格式错误"}
                return {
                    "state": True,
                    "data": {
                        "page_count": 1,
                        "tasks": [{"info_hash": "ABC", "status": 2, "percentDone": 100}],
                    },
                }
    ...
    def test_concurrent_invalid_token_failures_share_one_openlist_refresh(self):
        from pipeline.bot import BotConfig, PipelineBotService

        events = []
        token_store = RetryTokenStore()
        stale_calls_ready = threading.Barrier(2)

        class Concurrent115Client:
            def __init__(self, token):
                self.token = token
            ...
            def get_offline_tasks(self, page=1):
                events.append(("115_tasks", self.token, page))
                if self.token == "expired-token":
                    stale_calls_ready.wait(timeout=5)
        ...
        with patch("pipeline.bot.OpenListTokenProvider", return_value=FakeOpenListTokenProvider()), patch(
            ...
            "pipeline.bot.load_access_token_from_api", side_effect=lambda *args, **kwargs: token_store.load_access_token()
        ), patch("pipeline.bot.Client115", side_effect=Concurrent115Client):
            service = PipelineBotService(BotConfig("token", {700656624}, "/tmp/state.db"))
            results = []
            errors = []
            ...
            def load_status():
                try:
                    results.append(service.task_status("movie", "ABC"))
                except Exception as exc:
                    errors.append(exc)

            workers = [threading.Thread(target=load_status) for _ in range(2)]
        ...
        self.assertEqual([task["status_name"] for task in results], ["success", "success"])
        self.assertEqual(token_store.tokens, ["expired-token", "fresh-token"])
        self.assertEqual(events.count(("openlist", "/115/电影", True)), 1)
        self.assertEqual(events.count(("client", "fresh-token")), 1)

    def test_invalid_token_after_refresh_is_reported_without_refresh_loop(self):
        from pipeline.bot import BotConfig, PipelineBotService
        ...
        with patch("pipeline.bot.OpenListTokenProvider", return_value=FakeOpenListTokenProvider()), patch(
            ...
            "pipeline.bot.load_access_token_from_api", side_effect=lambda *args, **kwargs: token_store.load_access_token()
        ), patch("pipeline.bot.Client115", side_effect=AlwaysInvalid115Client):
            service = PipelineBotService(BotConfig("token", {700656624}, "/tmp/state.db"))
            with self.assertRaises(P115OpenAPIError) as raised:
                service.task_status("movie", "ABC")

        self.assertEqual(raised.exception.code, 40140125)
        self.assertEqual(token_store.tokens, ["expired-token", "fresh-token"])
        self.assertEqual(events.count(("openlist", "/115/电影", True)), 1)
        self.assertEqual(
            [event[:2] for event in events if event[0] == "115_tasks"],
            [("115_tasks", "expired-token"), ("115_tasks", "fresh-token")],
        )
    ...
    def test_prepare_subscription_staging_refreshes_openlist_when_115_token_is_invalid(self):
        ...
        class Directory115Client:
            ...
            def get_folder_info(self, folder_id):
                ...
                if self.token == "expired-token":
                    return {"state": False, "code": 40140126, "message": "access_token 校验失败"}
                return {
                    "state": True,
                    "code": 0,
    ...
    def test_sync_completed_adult_task_formats_code_before_mediastation_scan(self):
        ...
        self.assertLess(events.index(("scan",)), events.index(("subtitle_match", "adult", "downloaded folder", "media-1", "MIDA-304")))
    ...
    def test_subtitle_backfill_status_filter_only_processes_selected_status(self):
        from pipeline.bot import BotConfig, CandidateStore, PipelineBotService
        from pipeline.config import category_to_msg_library_root
        ...
        class FakeSubtitleMatcher:
            ...
            def match_task(self, category, title, task, force=False):
                ...
                return {
                    ...
                    "subtitle_match_count": 1,
                    "subtitle_match_source": "subtitlecat",
                    "subtitle_match_filename": "%s.srt" % task.get("openlist_adult_code"),
                }
        ...
        fake_msg = FakeMediaStationClient(
            list_response={
                "data": {
                    "items": [
                        {"id": "media-untried", "library_id": root["library_id"], "title": "MIDE-882"},
                        {"id": "media-failed", "library_id": root["library_id"], "title": "SSIS-218"},
                        {"id": "media-not-found", "library_id": root["library_id"], "title": "CAWD-292"},
                    ]
                }
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp, patch("pipeline.bot.MediaStationClient", return_value=fake_msg), patch(
            ...
            state_db = str(Path(tmp) / "state.db")
            store = CandidateStore(state_db)
            store.save_subtitle_backfill_record("media-failed", "SSIS-218", "SSIS-218", {"subtitle_match_status": "failed", "subtitle_match_error": "timeout"})
            store.save_subtitle_backfill_record(
                "media-not-found",
                "CAWD-292",
                "CAWD-292",
            ...
            result = service.subtitle_backfill_adult(limit=2, retry_attempted=True, status_filter="failed")

        self.assertEqual(result["status_filter"], "failed")
        self.assertEqual(result["scanned"], 3)
        self.assertEqual(result["attempted"], 1)
        self.assertEqual(result["matched"], 1)
        self.assertEqual(matcher.calls, [("adult", "SSIS-218", "media-failed", "SSIS-218", False)])
    ...
    def test_sync_completed_adult_task_ignores_legacy_failed_artwork_step(self):
        from pipeline.bot import BotConfig, PipelineBotService
    ...
    def test_sync_completed_task_rescans_when_saved_msg_media_was_deleted(self):
        ...
        fake_openlist = CleaningOpenList(
            {
                adult_root: [{"name": "ABF-363", "is_dir": True, "size": 0}],
                target_path: [
                    {"name": "ABF-363.mp4", "is_dir": False, "size": 3 * 1024 * 1024 * 1024, "hash_info": {"sha1": "SHA1"}}
                ],
            },
            events=events,
        )
